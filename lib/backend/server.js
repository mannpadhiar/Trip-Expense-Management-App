...
app.get('/trips/:tripId',async (req,res) =>{
    try{
        const trip = await Trip.findById({_id : req.params.tripId}).populate('createdBy', '_id name').populate('members', '_id name email')
        res.json(trip);
    }catch(error){
       res.status(400).json({error : error.message})
    }
...
app.get('/transactions/:tripId',async (req,res) =>{
    try{
        const transactions = await Transaction.find({tripId : req.params.tripId}).populate('paidBy', '_id name').populate("distributedTo.userId", "_id name email");
