...
app.get('/transactions/:tripId',async (req,res) =>{
    try{
        const transactions = await Transaction.find({tripId : req.params.tripId}).populate('tripId', 'name createdBy members').populate('paidBy', '_id name').populate('distributedTo', '_id name');
